...
function StepType({ form, update, canChooseScope }: { form: EventForm; update: (p: Partial<EventForm>) => void; canChooseScope: boolean }) { const codexScopes: { value: EventScope; label: string }[] = [{ value: 'district', label: 'Local' }, { value: 'national', label: 'Nacional' }]; return <div className="space-y-4"><div className="grid grid-cols-2 gap-2">{EVENT_TYPE_KEYS.map((type) => { const Icon = eventIcon(type); const active = form.type === type; return <button key={type} onClick={() => update({ type })} className={cn('rounded-md border p-3 text-left', active ? 'border-foreground bg-secondary' : 'border-border')}><Icon size={18} className="mb-2" /><span className="text-xs font-[Josefin_Sans]">{EVENT_TYPE_LABELS_PT[type]}</span></button>; })}</div>{canChooseScope ? <div className="grid grid-cols-2 gap-1">{codexScopes.map(({ value, label }) => <button key={value} onClick={() => update({ scope: value })} className={cn('rounded-full border px-2 py-1 text-xs', form.scope === value ? 'bg-foreground text-background border-foreground' : 'border-border')}>{label}</button>)}</div> : <p className="text-sm text-muted-foreground font-[Josefin_Sans]">Âmbito definido automaticamente.</p>}</div>; }
