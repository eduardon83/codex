...
function BookSearchDialog({ open, onOpenChange, planId, month, onAdded }: any) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Adicionar livro ao plano</DialogTitle></DialogHeader><div className="relative"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" value={query} onChange={e => setQuery(e.target.value)} placeholder="Título, autor ou ISBN" /></div><div className="max-h-80 overflow-y-auto">{results.map(b => <button key={b.id} onClick={() => add(b)} className="w-full flex items-center gap-3 py-3 border-b border-border text-left"><div className="flex-1 min-w-0"><p className="text-sm truncate">{b.title}</p><p className="text-xs text-muted-foreground truncate">{b.author}</p></div><Badge className="bg-emerald-600/20 text-emerald-700 border-emerald-600/30">Na tua biblioteca</Badge></button>)}</div></DialogContent></Dialog>;
}
