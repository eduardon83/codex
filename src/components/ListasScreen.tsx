...
function ReadingListsArea({ userId, onGoToSearchReadingLists, onCreatePlan }: { userId: string | null; onGoToSearchReadingLists?: () => void; onCreatePlan: () => void }) {
  ...
  return <div className="space-y-8"><p className="text-xs leading-relaxed text-muted-foreground">{helpText}</p><section><div className="flex items-center justify-between mb-3"><h2 className="font-serif text-xl">As minhas listas</h2><Button size="sm" onClick={() => setDialogOpen(true)}><Plus size={14} className="mr-1" />Nova lista</Button></div>{personal.length === 0 ? <div className="text-center py-10 border border-border rounded-md"><p className="text-sm text-muted-foreground mb-3">Ainda não criaste nenhuma lista.</p><Button onClick={() => setDialogOpen(true)}>+ Nova lista</Button></div> : <div className="space-y-3">{personal.map(l => <ListCard key={l.id} list={l} onView={() => openDetail(l, false)} onReload={load} />)}</div>}</section><section><h2 className="font-serif text-xl mb-3">Listas subscritas</h2>{subscribed.length === 0 ? <div className="text-center py-10 border border-border rounded-md"><p className="text-sm text-muted-foreground mb-3">Ainda não subscreveste nenhuma lista.</p><Button variant="outline" onClick={onGoToSearchReadingLists}>Descobrir listas no Procurar</Button></div> : <div className="space-y-3">{subscribed.map(l => <ListCard key={l.id} list={l} onView={() => openDetail(l, true)} onReload={load} subscribed />)}</div>}</section><Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent><DialogHeader><DialogTitle>Nova lista</DialogTitle></DialogHeader><div className="space-y-3"><Input placeholder="Nome da lista" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /><Textarea placeholder="Descrição (opcional)" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} /><div className="grid grid-cols-2 gap-2"><Button disabled={!form.name.trim()} onClick={() => { setForm(f => ({ ...f, import: false })); setPreview([]); createList(); }}>Criar vazia</Button><Button variant={form.import ? 'default' : 'outline'} onClick={() => setForm(f => ({ ...f, import: true }))}>Importar de ficheiro</Button></div>{form.import && <><ImportFileInline mode="list" preview={preview} setPreview={setPreview} /><Button className="w-full" disabled={!form.name.trim() || preview.length === 0} onClick={createList}>Guardar com livros importados</Button></>}</div></DialogContent></Dialog></div>;
}
